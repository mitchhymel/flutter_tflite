...
public class TflitePlugin implements MethodCallHandler {
  ...
  public void onMethodCall(MethodCall call, Result result) {
    ...
    } else if (call.method.equals("detectObjectOnImageGeneric")) {
      try {
        detectObjectOnImageGeneric((HashMap) call.arguments, result);
      } catch (Exception e) {
        result.error("Failed to run model", e.getMessage(), e);
      }
  ...
  private void saveTensorImageToFile(TensorImage tImage, String name, int imageSize) {
    ...
    String fileName = name + "_" + formatter.format(todayDate);
    String fullPath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/" + fileName + ".png";
    File dest = new File(fullPath);
    try (FileOutputStream out = new FileOutputStream(dest)) {
      outBm.compress(Bitmap.CompressFormat.PNG, 100, out); // bmp is your Bitmap instance
      // PNG is a lossless format, the compression factor (100) is ignored
      Log.v("TFPLUGIN", "Saved to " + fullPath);
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  private class RunForMultipleInputs extends TfliteTask {
    ...
    protected void runTflite() {
      ...
      for (int i = 0; i < tfLite.getOutputTensorCount(); i++) {
        ...
        this.outputAsBuff.put(i, TensorBuffer.createFixedSize(t.shape(), DataType.FLOAT32));
      }
